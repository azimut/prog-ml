...
def wxys = (wxs, map (\i -> loss features pizzas wxs[i] 0) (iota (length wxs)))

-- Loss (y-axis) over weights (x-axis). With 0 bias.
...
def meshgrid = linspace_2d 20 (history[1] - 10.0) (history[2] + 10.0) (history[4] - 100.0) (history[5] + 100.0)
def sxw = meshgrid |> flatten |> map (.0)
def syb = meshgrid |> flatten |> map (.1)

def szl =
  let f (x, y) = loss features pizzas x y
...
-- unset ztics;
-- set lt 1 lw 1 lc rgb "black"
-- set lt 2 lw 1 lc rgb "gray"
-- set size square
-- set label font ",10"; set xlabel "Weight"; set ylabel "Bias"; set zlabel "Loss";
-- set dgrid3d 20,20; set grid; set xyplane at 0; set hidden3d;
-- unset border; unset key;
-- set multiplot layout 2,2 title "Gradient Descent with MSE"
